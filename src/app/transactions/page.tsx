...
export default function TransactionsPage() {
	...
	const allLinesValid = lines.every(
		(line) => line.categoryId > 0 && line.amount > 0,
	);
	const isValid =
		lineSumCents === totalCents &&
		totalCents > 0 &&
		description.trim() &&
		allLinesValid;

	const editLineSumCents = editLines.reduce(
		(sum, line) => sum + Math.round(line.amount * 100),
		0,
	);
	const editTotalCents = Math.round((Number(editTotalAmount) || 0) * 100);
	const editAllLinesValid = editLines.every(
		(line) => line.categoryId > 0 && line.amount > 0,
	);
	const isEditValid =
		editLineSumCents === editTotalCents &&
		editTotalCents > 0 &&
  ...
  const [searchQuery, setSearchQuery] = useState('')
	...
	const handleEditSubmit = async (e: React.FormEvent) => {
		...
		if (res.ok) {
			const data = await res.json();
			setTransactions(
				transactions.map((t) => (t.id === editingId ? data.transaction : t)),
			);
			setEditingId(null);
		} else {
			const error = await res.json();
				...
				<form onSubmit={handleSubmit}>
					...
					<div className="mb-6">
						...
						<div className="mt-2 text-sm">
							<span
								className={
									lineSumCents === totalCents && totalCents > 0
										? "text-green-600"
										: "text-red-600"
								}
							>
								Line sum: ${(lineSumCents / 100).toFixed(2)} / Total: $
						...
						{transactions.map((transaction) => {
							...
							if (editingId === transaction.id) {
								return (
									<div
										key={transaction.id}
										className="border-2 border-blue-400 rounded p-4 bg-blue-50"
									>
										<h3 className="font-semibold mb-4">Edit Transaction</h3>
										<form onSubmit={handleEditSubmit}>
											<div className="mb-4">
											...
											<div className="mb-6">
												...
												{editLines.map((line, index) => (
													<div key={index} className="flex gap-2 mb-2">
														...
														<input
															...
															value={line.notes}
															onChange={(e) =>
																updateEditLine(index, "notes", e.target.value)
															}
														/>
												...
												<div className="mt-2 text-sm">
													...
													>
														Line sum: ${(editLineSumCents / 100).toFixed(2)} /
														Total: ${(editTotalCents / 100).toFixed(2)}
													</span>
												</div>
											</div>
