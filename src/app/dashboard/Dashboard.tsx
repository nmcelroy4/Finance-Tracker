		...
		transactions.forEach((transaction) => {
			transaction.lines.forEach((line) => {
				...
				categoryTotals[line.categoryId] =
					(categoryTotals[line.categoryId] || 0) + line.amount;
			});
		});
	...
	return (
		<div className="space-y-6">
			...
			<div className="grid  md:grid-cols-3 gap-4">
				<InfoTile
					title="Income"
					total={(stats.totalIncome / 100).toFixed(2)}
					variant="success"
				/>
				<InfoTile
					title="Expenses"
					total={(stats.totalExpenses / 100).toFixed(2)}
					variant="danger"
				/>
				<InfoTile
					title="Net Cashflow"
					total={(stats.netCashFlow / 100).toFixed(2)}
				/>
			</div>
				...
				{categoryChartData.length === 0 ? (
					...
					<div className="space-y-2">
						{categoryChartData.map((item, index) => {
							if (!item) return null;
							const percentage = (
								(item.value / (stats.totalExpenses / 100)) *
								100
							).toFixed(1);
