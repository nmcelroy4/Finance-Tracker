...
export default function NetWorthTile({
	...
	const loadAll = async () => {
		...
		try {
			...
			setSnapshots(
				Array.isArray(snapData?.snapshots) ? snapData.snapshots : [],
			);
		} catch (err) {
	...
	const openEditForm = (date: string) => {
		...
		const vals: Record<number, string> = Object.fromEntries(
			accounts.map((a) => [a.id, ""]),
		);
	...
	const months = [...new Set(snapshots.map((s) => s.snapshotDate))].sort(
							...
							{accounts.length === 0 ? (
								...
								accounts.map((a) => (
									...
									>
										...
										<span className="text-gray-500 mr-auto ml-3">
											{TYPE_LABELS[a.type]} · {a.category}
										</span>
										<Button
											variant="ghost"
											size="sm"
						...
						<div className="grid grid-cols-2 md:grid-cols-4 gap-2 items-end">
							...
							<div className="flex flex-col gap-1">
								...
								<select
									...
									onChange={(e) =>
										setNewAccount((v) => ({ ...v, category: e.target.value }))
									}
								>
							...
							<Button
								...
								disabled={savingAccount || !newAccount.name}
							>
								Add Account
							</Button>
						</div>
				...
				{showSnapshotForm && (
					<div className="mb-6 border rounded-md p-4">
							...
							{accounts.map((a) => (
								<div key={a.id} className="flex flex-col gap-1">
									...
									<Input
										...
										onChange={(e) =>
											setFormValues((v) => ({ ...v, [a.id]: e.target.value }))
										}
									/>
								</div>
							))}
						...
						<div className="flex justify-end gap-2">
							<Button variant="outline" size="sm" onClick={closeSnapshotForm}>
								Cancel
							</Button>
							<Button
								size="sm"
								onClick={handleSaveSnapshot}
								disabled={savingSnapshot || !formMonth}
							>
								Save Snapshot
							</Button>
						</div>
